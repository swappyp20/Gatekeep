...
**Gatekeep** is an MCP security proxy that protects Claude Desktop from Indirect Prompt Injection (IPI) attacks via Google Calendar events. It sits between Claude Desktop and Google Calendar, scanning all event data through a tiered detection engine before Claude sees it.

- **License:** Apache 2.0
- **Repository:** https://github.com/swappyp20/Gatekeep.git
...
- **Composition over modification:** The upstream google-calendar-mcp code lives in `src/upstream/` with minimal changes. Gatekeep wraps the `executeWithHandler` callback via `ProxyHandler` rather than modifying upstream tool handlers.
...
- `src/server.ts` — Integration seam with upstream ToolRegistry + Gatekeep MCP tools
...
- `GATEKEEP_READ_ONLY=true` — Disables write calendar tools by default
- `GATEKEEP_THREAT_INTEL=false` — Cloud threat intel (opt-in)
- `GATEKEEP_RISK_THRESHOLD_*` — Tunable detection thresholds

Local data stored in `~/.gatekeep/` (client-id, logs, quarantine, cache).

## Security Considerations

- Gatekeep's own tool inputs must be validated with Zod schemas — never interpolate user/event data into commands
